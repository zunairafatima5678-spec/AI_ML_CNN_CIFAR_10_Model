...
GDRIVE_FILE_ID = "https://drive.google.com/file/d/1_nd5hPpCZ4tjGucAtLRAjHf21RTXVsiR/view?usp=sharing"
...
# Load Model with caching + Google Drive Download
@st.cache_resource
def load_model():
    model_dir = os.path.join(os.path.dirname(__file__), "model")
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, "cnn_cifar10.keras")

    # اگر model موجود نہیں تو Drive سے download کرو
    if not os.path.exists(model_path):
        if GDRIVE_FILE_ID == "YOUR_FILE_ID_HERE":
            st.error("Please add your Google Drive File ID in GDRIVE_FILE_ID variable")
            st.stop()

        st.info("Downloading model from Google Drive... This will take 1-2 minutes on first run")
